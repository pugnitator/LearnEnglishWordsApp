package com.example.learnenglishwordsapp.trainer
import trainer.Question
import trainer.Statistics
import trainer.Word
import java.io.File
...
//            }
    ...
    private fun getListOfLearnedWords() =
        dictionary.filter { it.numberOfCorrectAnswers >= limitOfCorrectAnswers }

    private fun getListOfUnlearnedWords() =
        dictionary.filter { it.numberOfCorrectAnswers < limitOfCorrectAnswers }

    fun getNextQuestion(): Question? {
        var answerOptions =
            getListOfUnlearnedWords().shuffled().takeLast(numberOfWordsToDisplayed).toMutableSet()
        if (answerOptions.isEmpty()) currentQuestion = null
            ...
            if (answerOptions.size < numberOfWordsToDisplayed) {
                ...
                answerOptions.addAll(
                    getListOfLearnedWords().shuffled().takeLast(numberOfMissingWords)
                )
    ...
    fun isAnswerCorrect(userAnswer: Int?): Boolean {
        ...
        if (currentQuestion == null) isAnswerCorrect = false
            ...
            val correctAnswer =
                currentQuestion!!.answerOptions.indexOf(currentQuestion!!.wordToStudy)
